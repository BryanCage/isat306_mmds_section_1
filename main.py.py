...
lcd = Adafruit_CharLCD(rs=21, en=19, d4=13, d5=6, d6=5, d7=12, cols=16, lines=2)
...
def knobPressed(channel):
    ...
    if (encoder == encode_Blue and not mode_Blue):  # 1
        ...
        with open('config.json') as config_file:
            config = json.load(config_file)
            config['scale']['units']['g'] = False
            config['scale']['units']['lbs'] = False
            config['scale']['units']['oz'] = True
            config_file.close()
            config_file = open('config.json', 'w')
            config_file.write(json.dumps(config, indent=4, sort_keys=True))
            config_file.close()
...
def main():
    ...
    GPIO.add_event_detect(27, GPIO.RISING, callback=knobPressed, bouncetime=200)
